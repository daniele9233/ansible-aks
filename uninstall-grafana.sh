#!/bin/bash
# Disinstalla Grafana 11 installato dal role grafana_install.
# Rimuove: Deployment, Service, Ingress, Secrets, PVC pvc-grafana nel
# namespace 'monitoring'. NON cancella il namespace se contiene altri
# carichi (es. pgAdmin). La PVC managed-csi ha reclaim policy Delete ->
# AKS rimuove anche il disco Azure sottostante automaticamente.
#
# Idempotente: puo' essere rilanciato a freddo o dopo un install fallito.
# NON tocca il cluster AKS, ne' altri namespace, ne' altri carichi
# (es. pgAdmin) presenti nel namespace 'monitoring'.
#
# Dipendenze: kubectl
...
log "INIZIO DISINSTALLAZIONE Grafana (namespace: $NS)"
...
# ---------------------------------------------------------------------------
# 2. Delete delle risorse Grafana tramite label selector
...
# ---------------------------------------------------------------------------
log "3/5 Delete risorse Grafana per nome (fallback)"
kubectl -n "$NS" delete deploy   grafana              --ignore-not-found
kubectl -n "$NS" delete svc      grafana              --ignore-not-found
kubectl -n "$NS" delete ingress  grafana              --ignore-not-found
kubectl -n "$NS" delete secret   tls-grafana-ingress  --ignore-not-found
kubectl -n "$NS" delete secret   grafana-admin        --ignore-not-found
kubectl -n "$NS" delete pvc      pvc-grafana          --ignore-not-found
...
for i in $(seq 1 12); do
  if ! kubectl -n "$NS" get pvc pvc-grafana >/dev/null 2>&1; then
...
# ---------------------------------------------------------------------------
# 5. Cancella il namespace SOLO se vuoto (rispetta pgAdmin e altri carichi)
# ---------------------------------------------------------------------------
...
if [ "$remaining" -eq 0 ]; then
  ...
  echo "  - $NS contiene altre $remaining risorse (es. pgAdmin), lo lascio intatto"
  echo "    (per cancellarlo manualmente: kubectl delete namespace $NS)"
fi

log "DISINSTALLAZIONE Grafana COMPLETATA"
echo "Per reinstallare: ansible-playbook site.yml --tags grafana --ask-vault-pass"
